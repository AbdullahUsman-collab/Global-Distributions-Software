...
class FinancialReportService {
    ...
    /* ─── Trial Balance ─────────────────────────────────────── */
    async generateTrialBalance(filter) {
    ...
    /* ─── Profit & Loss ─────────────────────────────────────── */
    async generateProfitAndLoss(filter) {
        const accounts = await this.coaRepo.getAccountsByTenantId(filter.tenantId);
        const allEntries = await this.voucherRepo.getLedgerEntries(filter.tenantId, { status: 'POSTED' });
        // Only period activity (no opening for P&L — P&L is period-based)
        const periodEntries = allEntries.filter(e => e.entryDate >= filter.startDate && e.entryDate <= filter.endDate);
        // Build account map
        const accountByCode = new Map();
        for (const a of accounts) {
            accountByCode.set(a.accountCode, a);
        ...
        for (const account of accounts) {
            ...
            if (amount === 0 && !filter.showZeroBalance)
                continue;
            const row = {
                accountId: account.id,
                accountCode: account.accountCode,
    ...
    /* ─── Balance Sheet ─────────────────────────────────────── */
    async generateBalanceSheet(filter) {
        const accounts = await this.coaRepo.getAccountsByTenantId(filter.tenantId);
        ...
        const accountAncestors = new Map(); // accountId -> top-level ancestor
        for (const account of accounts) {
            if (account.level === 1) {
        ...
        for (const account of accounts) {
            ...
            if (balance === 0 && !filter.showZeroBalance)
                continue;
            const row = {
                accountId: account.id,
                accountCode: account.accountCode,
            ...
            else if (ancestor.legacyMainHeadNo === 500 ||
    ...
    isDescendant(candidate, ancestor, allAccounts) {
