...
class BillDetailService {
    constructor(voucherRepo, coaRepo, customerRepo, supplierRepo, inventoryRepo) {
     ...
     */
    async getBillDetail(tenantId, voucherId) {
        ...
        for (const line of lines) {
            ...
            if (customer) {
                ...
                if (acc)
                    partyAccountCode = acc.accountCode;
                break;
            }
        ...
        const accountingEntries = voucherLedger.map(entry => {
        ...
        const inventoryMovements = voucherMovements.map(m => {
        ...
        return {
            ...
            stockLevels: stockLevels.filter(sl => billLines.some(bl => bl.line.productId === sl.productId)),
        };
    }
}
