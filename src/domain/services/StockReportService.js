...
class StockReportService {
    ...
    async generateStockBWA(filter) {
        ...
        for (const mov of postedMovements) {
            ...
            if (!isBeforeStart && !isInRange)
                continue;
            const target = isBeforeStart ? act : act;
            switch (mov.movementType) {
                case 'GRN':
                    if (isBeforeStart)
                        target.openingGrn += mov.quantity;
                    else
                        target.periodGrn += mov.quantity;
                    break;
                case 'ISSUE':
                    if (isBeforeStart)
                        target.openingIssue += mov.quantity;
                    else
                        target.periodIssue += mov.quantity;
                    break;
                case 'RETURN':
                    if (isBeforeStart)
                        target.openingReturn += mov.quantity;
                    else
                        target.periodReturn += mov.quantity;
                    break;
                case 'TRANSFER': {
                    ...
                    if (isBeforeStart) {
                        if (isDestination)
                            target.openingTransferIn += mov.quantity;
                        else if (isSource)
                            target.openingTransferOut += mov.quantity;
                        else {
                            target.openingTransferIn += mov.quantity;
                            target.openingTransferOut += mov.quantity;
                        }
                        ...
                        if (isDestination)
                            target.periodTransferIn += mov.quantity;
                        else if (isSource)
                            target.periodTransferOut += mov.quantity;
                        else {
                            target.periodTransferIn += mov.quantity;
                            target.periodTransferOut += mov.quantity;
                        }
                ...
                case 'ADJUSTMENT': {
                    ...
                    if (isBeforeStart) {
                        if (isPositive)
                            target.openingAdjPos += mov.quantity;
                        else
                            target.openingAdjNeg += Math.abs(mov.quantity);
                    }
                    else {
                        if (isPositive)
                            target.periodAdjPos += mov.quantity;
                        else
                            target.periodAdjNeg += Math.abs(mov.quantity);
                    }
                    break;
                }
            }
        ...
        for (const p of targetProducts) {
            ...
            const openingQty = r2(act.openingGrn + act.openingReturn + act.openingAdjPos + act.openingTransferIn
            ...
            const closingQty = r2(openingQty + grnQty + returnQty + adjustmentQty + transferInQty - issueQty - transferOutQty);
            rows.push({
                productId: p.id,
                productCode: p.sku,
                productName: p.name,
