...
const CASH_VOUCHER_TYPES = new Set([
    'CR', // Cash Receipt
    'CP', // Cash Payment
    'CV', // Cash Voucher
...
class CashBookService {
    constructor(coaRepo, voucherRepo) {
     ...
     */
    async getCashBankAccounts(tenantId) {
     ...
     */
    async getCashBook(tenantId, accountHeadId, startDate, endDate) {
        ...
        const cashVouchers = allVouchers.filter(v => CASH_VOUCHER_TYPES.has(v.voucherType) && v.date >= startDate && v.date <= endDate);
        ...
        const draftVouchers = allVouchers.filter(v => v.status === 'DRAFT' && CASH_VOUCHER_TYPES.has(v.voucherType));
        return {
            account,
            openingBalance,
     ...
     */
    async createCashReceipt(tenantId, dto, createdBy, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.CASH_CREATE);
        if (dto.amount <= 0)
        ...
        return this.voucherRepo.createVoucher(tenantId, {
            voucherType: 'CR',
            date: dto.date,
     ...
     */
    async createCashPayment(tenantId, dto, createdBy, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.CASH_CREATE);
        if (dto.amount <= 0)
            throw new Error('Amount must be greater than zero');
        if (!dto.narration.trim())
     ...
     */
    async postVoucher(tenantId, voucherId, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.CASH_POST);
        return this.voucherRepo.postVoucher(tenantId, voucherId);
    }
    /**
     * Delete a DRAFT cash book voucher.
     */
    async deleteVoucher(tenantId, voucherId, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.CASH_DELETE);
     ...
     */
    async getDraftVouchers(tenantId) {
