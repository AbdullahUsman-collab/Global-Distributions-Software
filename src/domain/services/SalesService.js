...
class SalesService {
     ...
     */
    async createSaleBill(tenantId, dto, createdBy, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.SALES_CREATE);
        ...
        for (const line of dto.lines) {
            ...
            totalCogs += (0, inventory_1.calculateCOGS)(line.packs, costRate);
        }
     ...
     */
    async postSaleBill(tenantId, voucherId, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.SALES_POST);
        ...
        for (const line of voucherLines) {
            if (line.productId && line.quantity && line.quantity > 0) {
                ...
                const lineCogs = (0, inventory_1.calculateCOGS)(line.quantity, costRate);
                totalCogs += lineCogs;
                if (sourceLevel) {
     ...
     */
    async deleteSaleBill(tenantId, id, role = 'ADMIN') {
        (0, AuthorizationService_1.requirePermission)(role, AuthorizationService_1.Permissions.SALES_DELETE);
