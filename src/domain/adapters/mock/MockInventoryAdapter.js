...
const warehousesStore = new Map();
const locationsStore = new Map();
const stockLevelsStore = new Map();
const movementsStore = new Map();
const batchesStore = new Map();
const serialsStore = new Map();
/* ─── Seed Data ────────────────────────────────────────────── */
/**
 ...
 */
function buildSeedProducts(tenantId) {
    ...
    return rawProducts.map(p => ({
        ...p,
        costRate: (0, inventory_1.calculateCostRate)(p.purchaseRate, p.retailPrice, MARGIN),
        margin: MARGIN,
    }));
}
/**
 ...
 */
function buildSeedWarehouses(tenantId) {
    return [
 ...
 */
function buildSeedLocations(tenantId, warehouseId, warehouseCode) {
    return [
 ...
 */
function buildSeedStockLevels(tenantId, products, warehouses) {
    const levels = [];
 ...
 */
function buildSeedMovements(tenantId, products, warehouses) {
    const movements = [];
    const now = new Date();
...
class MockInventoryAdapter {
    ...
    async createProduct(tenantId, dto) {
        ...
        const costRate = dto.costRate ?? (0, inventory_1.calculateCostRate)(dto.purchaseRate ?? 0, dto.retailPrice ?? 0, margin);
        const product = {
            id: uid(),
            tenantId,
            sku: dto.sku,
    ...
    async updateProduct(tenantId, id, dto) {
        ...
        const newCostRate = dto.costRate ?? (0, inventory_1.calculateCostRate)(newPurchaseRate, newRetailPrice, newMargin);
        const updated = {
            ...existing,
    ...
    async deactivateProduct(tenantId, id) {
        const products = productsStore.get(tenantId) ?? [];
        const idx = products.findIndex(p => p.id === id);
        if (idx === -1)
            throw new Error('Product not found');
    ...
    async getStockLevelForProduct(tenantId, productId, warehouseId) {
    ...
    async postStockMovement(tenantId, movementId) {
        const movements = movementsStore.get(tenantId) ?? [];
        ...
        if (movement.status === 'POSTED')
            throw new Error('Movement is already posted');
        if (movement.status === 'CANCELLED')
            throw new Error('Cannot post a cancelled movement');
        const levels = stockLevelsStore.get(tenantId) ?? [];
        const products = productsStore.get(tenantId) ?? [];
        ...
        switch (movement.movementType) {
            ...
            case 'RETURN': {
                ...
                if (!targetWarehouseId)
                    throw new Error('Target warehouse required for GRN/RETURN');
                const levelIdx = levels.findIndex(l => l.productId === movement.productId && l.warehouseId === targetWarehouseId);
                if (levelIdx === -1) {
                    ...
                    const newUnitCost = (0, inventory_1.calculateAVCO)(level.quantityOnHand, level.unitCost, movement.quantity, movement.unitCost);
                    levels[levelIdx] = {
                        ...level,
            ...
            case 'ISSUE': {
                ...
                if (!sourceWarehouseId)
                    throw new Error('Source warehouse required for ISSUE');
                const levelIdx = levels.findIndex(l => l.productId === movement.productId && l.warehouseId === sourceWarehouseId);
                if (levelIdx === -1) {
                ...
                if (level.quantityOnHand < movement.quantity) {
                    throw new Error(`Insufficient inventory stock for Product [${product.name}] — ` +
            ...
            case 'TRANSFER': {
                ...
                const sourceIdx = levels.findIndex(l => l.productId === movement.productId && l.warehouseId === sourceWarehouseId);
                if (sourceIdx === -1) {
                ...
                if (sourceLevel.quantityOnHand < movement.quantity) {
                    throw new Error(`Insufficient inventory stock for Product [${product.name}] in source warehouse — ` +
                ...
                const targetIdx = levels.findIndex(l => l.productId === movement.productId && l.warehouseId === targetWarehouseId);
                if (targetIdx === -1) {
                    levels.push({
                        id: uid(),
                        tenantId,
                    ...
                    const newUnitCost = (0, inventory_1.calculateAVCO)(targetLevel.quantityOnHand, targetLevel.unitCost, movement.quantity, sourceLevel.unitCost);
                    levels[targetIdx] = {
                        ...targetLevel,
            ...
            case 'ADJUSTMENT': {
                ...
                const levelIdx = levels.findIndex(l => l.productId === movement.productId && l.warehouseId === warehouseId);
                if (levelIdx === -1) {
    ...
    async cancelStockMovement(tenantId, movementId) {
        const movements = movementsStore.get(tenantId) ?? [];
        const idx = movements.findIndex(m => m.id === movementId);
        if (idx === -1)
            throw new Error('Stock movement not found');
        const movement = movements[idx];
        if (movement.status === 'POSTED')
            throw new Error('Cannot cancel a posted movement');
        if (movement.status === 'CANCELLED')
