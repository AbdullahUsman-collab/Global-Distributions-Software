...
exports.MockCOAAdapter = void 0;
const coa_1 = require("../../types/coa");
/* ─── Helpers ──────────────────────────────────────────────── */
let nextId = 1000;
function uid() {
    return `coa-${nextId++}`;
}
function makeAccount(tenantId, code, name, level, accountType, parentId, controlCategory, legacyMainHeadNo, accountEffect) {
    const normalBalance = (0, coa_1.deriveNormalBalance)(accountType);
    return {
        id: uid(),
        tenantId,
        accountCode: code,
...
/* ─── Seed Data Builder ────────────────────────────────────── */
function buildSeedTree(tenantId) {
    const accounts = [];
    function add(code, name, level, type, parentCode, cc, legacyNo, effect) {
...
class MockCOAAdapter {
    async getAccountsByTenantId(tenantId) {
    ...
    async createAccount(tenantId, dto) {
        ...
        const normalBalance = (0, coa_1.deriveNormalBalance)(dto.accountType);
        const account = {
            id: uid(),
            tenantId,
    ...
    async deactivateAccount(tenantId, id) {
        const accounts = store.get(tenantId) ?? [];
        const index = accounts.findIndex(a => a.id === id);
        if (index === -1)
            throw new Error('Account not found');
...
exports.MockCOAAdapter = MockCOAAdapter;
