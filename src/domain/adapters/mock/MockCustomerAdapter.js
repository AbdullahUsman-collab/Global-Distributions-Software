...
exports.MockCustomerAdapter = void 0;
/* ─── Helpers ──────────────────────────────────────────────── */
let nextId = 9000;
function uid() {
    return `cust-${nextId++}`;
 ...
 */
function buildSeedCustomers(tenantId, accountHeadIds) {
...
class MockCustomerAdapter {
    ...
    /* ─── Queries ─────────────────────────────────────────────── */
    async getCustomersByTenantId(tenantId, filters) {
    ...
    async getCustomerByAccountHeadId(tenantId, accountHeadId) {
        const customers = customersStore.get(tenantId) ?? [];
    ...
    async deactivateCustomer(tenantId, id) {
        const customers = customersStore.get(tenantId) ?? [];
        const index = customers.findIndex(c => c.id === id);
        if (index === -1)
            throw new Error('Customer not found');
    ...
    async searchCustomers(tenantId, prefix) {
        const customers = customersStore.get(tenantId) ?? [];
...
exports.MockCustomerAdapter = MockCustomerAdapter;
