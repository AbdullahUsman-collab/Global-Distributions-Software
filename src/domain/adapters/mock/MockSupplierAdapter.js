...
exports.MockSupplierAdapter = void 0;
/* ─── Helpers ──────────────────────────────────────────────── */
let nextId = 7000;
function uid() {
    return `supp-${nextId++}`;
...
class MockSupplierAdapter {
    ...
    async initializeSeeds() {
        for (const tid of TENANT_IDS) {
            ...
            const suppliers = seedSuppliers.map((sd) => {
    ...
    async getByAccountHeadId(accountHeadId, tenantId) {
        const suppliers = suppliersStore.get(tenantId) || [];
        const found = suppliers.find(s => s.accountHeadId === accountHeadId);
        return found ? deepClone(found) : null;
    }
    async create(supplierDTO, tenantId) {
        const suppliers = suppliersStore.get(tenantId) || [];
        const id = uid();
        const now = new Date();
        ...
        await this.coaRepo.createAccount(tenantId, {
    ...
    async update(id, supplierDTO, tenantId) {
        const suppliers = suppliersStore.get(tenantId) || [];
    ...
    async search(query, tenantId) {
        ...
        return suppliers.filter(s => s.isActive && (s.name.toLowerCase().includes(lowerQuery) ||
            (s.contactPerson?.toLowerCase().includes(lowerQuery) ?? false) ||
            (s.city?.toLowerCase().includes(lowerQuery) ?? false))).map(s => deepClone(s));
    }
}
exports.MockSupplierAdapter = MockSupplierAdapter;
