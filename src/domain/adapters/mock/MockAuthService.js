...
const MockUserCredentialsAdapter_1 = require("./MockUserCredentialsAdapter");
/**
 * Mock implementation of IAuthService.
 * DEVELOPMENT ONLY - Do not use in production.
 */
class MockAuthService {
    constructor(tenantRepository, userRepository, credentialsRepository, sessionRepository, brandAccessRepository) {
    ...
    async authenticate(credentials) {
        ...
        const userCredentials = await this.credentialsRepository.getCredentialsByUserId(user.id);
        if (!userCredentials) {
            return { success: false, error: 'Invalid credentials' };
        }
        ...
        const plainPassword = MockUserCredentialsAdapter_1.DEMO_PLAIN_PASSWORDS[user.id];
        ...
        const session = await this.sessionRepository.createSession(tenantId, user.id);
    ...
    async refreshSession(sessionId) {
        ...
        return this.sessionRepository.createSession(session.tenantId, session.userId);
    }
    /**
    ...
    async switchTenant(sessionId, targetTenantId) {
        ...
        await this.sessionRepository.deleteSession(sessionId);
        // 6. Create new session with target tenantId
        const newSession = await this.sessionRepository.createSession(targetTenantId, user.id);
        // 7. Return user with access-derived role for new tenant
        const authorizedUser = {
            ...user,
     ...
     */
    async getAuthorizedTenants(userId) {
    ...
    async changePassword(userId, payload) {
        ...
        const storedPlain = MockUserCredentialsAdapter_1.DEMO_PLAIN_PASSWORDS[userId];
        ...
        (0, MockUserCredentialsAdapter_1.registerTestPassword)(userId, newPassword);
