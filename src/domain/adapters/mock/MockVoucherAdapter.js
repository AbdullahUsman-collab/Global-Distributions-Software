...
exports.MockVoucherAdapter = void 0;
const voucher_1 = require("../../types/voucher");
/* ─── Helpers ──────────────────────────────────────────────── */
let nextId = 5000;
function uid() {
...
const linesStore = new Map();
const ledgerStore = new Map();
const voucherCounter = new Map();
function buildSeedVouchers(tenantId) {
    return [
        // ─── Opening Balance ──────────────────────────────────────
        {
            voucherType: 'JV',
 ...
 */
class MockVoucherAdapter {
    /* ─── Voucher Header Queries ───────────────────────────── */
    async getVouchersByTenantId(tenantId, filters) {
    ...
    /* ─── Mutations ────────────────────────────────────────── */
    async createVoucher(tenantId, dto, createdBy) {
    ...
    async updateVoucher(tenantId, id, dto) {
        const headers = headersStore.get(tenantId) ?? [];
    ...
    async postVoucher(tenantId, id) {
        ...
        // Validate balanced
        if (!(0, voucher_1.isBalanced)(voucherLines)) {
        ...
        // Update status
        const updated = { ...header, status: 'POSTED', updatedAt: new Date() };
        headers[idx] = updated;
        headersStore.set(tenantId, headers);
        return { ...updated };
    }
    /* ─── Ledger Queries ───────────────────────────────────── */
    async getLedgerEntries(tenantId, filters) {
    ...
    async getLedgerForAccount(tenantId, accountId, filters) {
...
exports.MockVoucherAdapter = MockVoucherAdapter;
